...
/*   Updated: 2023/04/04 13:57:52 by julien           ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_output_file(t_data *data)
{
	...
	// outfile = open((char *)s_read_cnt(data->cmd)->output[0], O_WRONLY | O_APPEND | O_TRUNC, 0644);
	if (outfile == -1)
...
void	ft_output_appends(t_data *data)
{
	...
	// outfile = open((char *)s_read_cnt(data->cmd)->output[0], O_WRONLY | O_CREAT | O_TRUNC, 0644);
	if (outfile == -1)
		perror("Error when creating or accessing outfile! ");
	if (dup2(outfile, STDOUT_FILENO) == -1)
	{
